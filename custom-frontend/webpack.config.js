...
const getProxyConfig = require('../superset-frontend/webpack.proxy-config');
...
const plugins = [
  ...
  new CopyPlugin({
    patterns: [
      'package.json',
      { from: `${ORIGINAL_FRONTEND_DIR}/src/assets/images`, to: 'images' },
      { from: `${ORIGINAL_FRONTEND_DIR}/src/assets/stylesheets`, to: 'stylesheets' },
    ],
  }),

  // static pages
  new HtmlWebpackPlugin({
    template: `${ORIGINAL_FRONTEND_DIR}/src/assets/staticPages/404.html`,
    inject: true,
    chunks: [],
    filename: '404.html',
  }),
  new HtmlWebpackPlugin({
    template: `${ORIGINAL_FRONTEND_DIR}/src/assets/staticPages/500.html`,
    inject: true,
    chunks: [],
    filename: '500.html',
  }),
...
const PREAMBLE = [path.join(ORIGINAL_FRONTEND_DIR, '/src/preamble.ts')];
if (isDevMode) {
...
function addPreamble(entry) {
  return PREAMBLE.concat([path.join(ORIGINAL_FRONTEND_DIR, entry)]);
}
...
const config = {
  ...
  resolve: {
    ...
    modules: ['node_modules', ORIGINAL_FRONTEND_DIR, CUSTOM_FRONTEND_DIR],
    alias: {
  ...
  context: CUSTOM_FRONTEND_DIR, // to automatically find tsconfig.json
  module: {
    rules: [
      {
        test: /datatables\.net.*/,
      ...
      },
      {
        ...
        exclude: [/superset-ui.*\/node_modules\//, /\.test.jsx?$/, `${ORIGINAL_FRONTEND_DIR}/src/visualizations/presets/MainPreset.js`],
        include: [
          new RegExp(`${ORIGINAL_FRONTEND_DIR}/(src|.storybook|plugins|packages)`),
          ...[`${ORIGINAL_FRONTEND_DIR}/src`, `${ORIGINAL_FRONTEND_DIR}/.storybook`, `${ORIGINAL_FRONTEND_DIR}/plugins`, `${ORIGINAL_FRONTEND_DIR}/packages`].map(p =>
            path.resolve(__dirname, p),
          ), // redundant but required for windows
          new RegExp(`${CUSTOM_FRONTEND_DIR}/(src|plugins)`),
          ...[`${CUSTOM_FRONTEND_DIR}/src`, `${CUSTOM_FRONTEND_DIR}/plugins`].map(p =>
            path.resolve(__dirname, p),
          ), // redundant but required for windows
          /@encodable/,
        ],
      ...
      {
        test: /\.css$/,
        include: [ORIGINAL_FRONTEND_DIR, /superset-ui.+\/src/, CUSTOM_FRONTEND_DIR],
        use: [
      ...
      {
        test: /\.less$/,
        include: [ORIGINAL_FRONTEND_DIR, CUSTOM_FRONTEND_DIR],
        use: [
          isDevMode ? 'style-loader' : MiniCssExtractPlugin.loader,
          {
            loader: 'css-loader',
...
Object.entries(packageConfig.dependencies).forEach(([pkg, relativeDir]) => {
  const srcPath = path.join(CUSTOM_FRONTEND_DIR, `./node_modules/${pkg}/src`);

  if(relativeDir.startsWith('file:./')) {
    ...
    if (/^@superset-ui/.test(pkg) && fs.existsSync(srcPath)) {
      ...
      config.resolve.alias[pkg] = path.resolve(CUSTOM_FRONTEND_DIR, `${dir}/src`);
    }
  } else {
    const dir = relativeDir.replace('file:', '');
    ...
    if (/^@superset-ui/.test(pkg) && fs.existsSync(srcPath)) {
      console.log(`[Superset Plugin] Use symlink source for ${pkg} @ ${dir}`);
      config.resolve.alias[pkg] = path.resolve(ORIGINAL_FRONTEND_DIR, `${dir}/src`);
    }
  }
});
